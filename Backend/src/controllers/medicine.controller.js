...
export const addMedicine = asyncHandler(async (req, res) => {
  ...
  const medicine = await Medicine.create({ userId, name, dosage, frequency, duration, startDate, notes, reminderEnabled, refillReminder, currentSupply, refillAt, totalSupply: currentSupply });
  ...
  res.status(StatusCodes.CREATED).send({ status: true, message: "Medicine created successfully with all doses in a single log", data: { _id: medicine._id } });
});
...
export const getHistory = asyncHandler(async (req, res) => {
  ...
  // Fetch logs for these medicines
  ...
  // Map logs to medicines with real-time status update
...
export const markDoseTaken = asyncHandler(async (req, res) => {
  ...
  res.status(200).send({ status: true, message: "Medicine taken successfully", currentSupply: medicine?.currentSupply });
});
